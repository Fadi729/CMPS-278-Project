...
import AddToWishListButton from '../WishListButton';
...
const BookPage: React.FC = () => {
  ...
  const filteredBook = myBooks.find((book: { id: string | undefined; }) => book.id === id)!;
  const moreBooks = myBooks.filter((book: { categories: string | null }) => book.categories === filteredBook.categories);
  const filteredReviews = myBookReviews.filter((review: { book_Id: string | undefined; }) => review.book_Id === id);

  const bookList = moreBooks.map((book) => {
  ...
  const formattedDate = filteredBook.publishedDate;
    ...
    <div>
      <div id="main" className='font-google relative left-16 top-24 mt-6 flex'>
        ...
        <div id="details" className=' w-[50%]'>
          <h1 className=' text-6xl font-bold'>{filteredBook.title}</h1>
          {filteredBook.authors!.map((author: string | number | boolean | React.ReactElement<any, string | React.JSXElementConstructor<any>> | React.ReactFragment | React.ReactPortal | null | undefined) => (
          ...
          <div id='buttons' className='mt-8'>
            ...
            <WishListButton item={{itemId: filteredBook.id, itemType: ItemType.Book}}/>

          </div>
        </div>

        <div id="photo" className='w-[247px] h-[345px] ml-96'>
          <img src={filteredBook.image!} alt="" />
        </div>
      ...
      <div className='font-google relative left-16 top-24 mt-6 flex flex-row '>
        <div id="about" className=' w-[65%]'>
          ...
          <button className="bg-gray-100 text-gray-800 font-bold py-2 px-4 rounded-full mt-5">
            {filteredBook.categories!.substring(2, filteredBook.categories!.length - 2)}
          </button>
