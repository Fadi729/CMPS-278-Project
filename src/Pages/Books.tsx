...
const Books = () => {
  ...
  // const books: BooksData[] = bookData.map((book) => ({
  //   id: book.Id,
  //   title: book.Title,
  ...
  // const bookReviews: BooksReview[] = BooksReviews.map((review) => ({
  //   id: review.Id,
  ...
  //   reviewhelpfulness: review.reviewhelpfulness,
  ...
  const bookList = books.map((book) => {
    ...
    const ratingSum = bookRvws.reduce(
  ...
  return (
    <>
      {isLoading ? (
        <div className="text-2xl font-google relative left-16 top-24">
          Loading...
        </div>
      ) : (
        <>
          <h1 className="text-2xl font-google relative left-16 top-24">
            Browse books
          </h1>
...
export const bookReviews = (state: { Books: { bookReviews: any } }) =>
  state.Books.bookReviews;
